...
const FeatureSection = () => {
  ...
  return (
    <section id="features" className="container mx-auto mt-8 ">
      ...
      >
        <span
          className="inline-flex items-center gap-1.5 rounded-full border border-primary  px-3 py-1 text-xs text-muted-foreground"
        >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {features.map((feature, idx) => (
          <UICard key={feature.title} index={idx} data={feature} />
        ))}
      </div>
    </section>
  )
